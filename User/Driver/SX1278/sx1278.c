...
void SX1278_hw_init(SX1278_hw_t * hw) {
	...
	gpio_bit_set(hw->reset.port, hw->reset.pin);
}

void SX1278_hw_SetNSS(SX1278_hw_t * hw, int value) {
	if(value == 1){
		gpio_bit_set(hw->nss.port, hw->nss.pin);
	}
	else{
		gpio_bit_reset(hw->nss.port, hw->nss.pin);
	}
}

void SX1278_hw_Reset(SX1278_hw_t * hw) {
	SX1278_hw_SetNSS(hw, 1);
	gpio_bit_reset(hw->reset.port, hw->reset.pin);
...
//	SX1278_hw_SetNSS(hw, 0);
	spi_i2s_data_transmit(hw->spi, cmd);
	while ((spi_i2s_flag_get(hw->spi,SPI_FLAG_TRANS) == SET)&&(Get_Tick()-tickstart<1000));
	while (spi_i2s_flag_get(hw->spi,SPI_FLAG_TBE) == RESET);
}
...
//	SX1278_hw_SetNSS(hw, 0);
	while (spi_i2s_flag_get(hw->spi,SPI_FLAG_TBE) == RESET);
	spi_i2s_data_transmit(hw->spi, 10);
	rxByte = spi_i2s_data_receive(hw->spi);
	while (spi_i2s_flag_get(hw->spi,SPI_FLAG_TRANS) == SET);
//	while (spi_i2s_flag_get(hw->spi,SPI_FLAG_RBNE) == RESET);
//			SX1278_hw_DelayMs(1);
	rxByte = spi_i2s_data_receive(hw->spi);
//		temp_L = rxByte & 0x00ff;
//		temp_H = (rxByte & 0xff00)>>8; 
	while ((spi_i2s_flag_get(hw->spi,SPI_FLAG_RBNE) ) == SET&&(Get_Tick()-tickstart<1000));
//		SX1278_hw_SetNSS(hw, 1);
	return rxByte;
}
...
//	return gpio_input_bit_get(hw->dio0.port,hw->dio0.pin);
		return gpio_input_bit_get(LORA_DIO0_GPIO_PROT,LORA_DIO0_PIN);
}
...
void SX1278_config(SX1278_t * module, uint8_t frequency, uint8_t power,
	...
	SX1278_entryLoRa(module);     //lora mode    fsk mode
				mode = SX1278_SPIRead(module, LR_RegOpMode);
//			mode = SX1278_SPIRead(module, LR_RegOpMode);
...
//	mode = SX1278_SPIRead(module, LR_RegOpMode);
	...
	SX1278_SPIWrite(module, LR_RegPreambleLsb, 6); //RegPreambleLsb 8+4=12byte Preamble
...
//	addr = SX1278_SPIRead(module, LR_RegFifoRxBaseAddr); //Read RxBaseAddr
		addr = SX1278_SPIRead(module, 0x0e); //Read RxBaseAddr
...
//	UNUSED(temp);
	...
	while (1) {
		if (gpio_input_bit_get(LORA_DIO0_GPIO_PROT,LORA_DIO0_PIN) == SET) { //if(Get_NIRQ()) //Packet send over
//		if (SX1278_hw_GetDIO0(module->hw)) { //if(Get_NIRQ()) //Packet send over
			temp = SX1278_SPIRead(module, LR_RegIrqFlags);
...
uint8_t SX1278_RSSI_LoRa(SX1278_t * module) {
	uint32_t temp = 10;
	temp = SX1278_SPIRead(module, LR_RegOpMode); //Read RegRssiValue, Rssi value
	temp = temp ; //127:Max RSSI, 137:RSSI offsettemp + 127 - 137;
	return (uint8_t) temp;
}
...
void SX1278_LORA_CAD(SX1278_t * module){
	
	SX1278_defaultConfig(module);
	SX1278_SPIWrite(module, REG_LR_DIOMAPPING1, 0xa1);	//DIO0=10, DIO1=10, DIO2=00, DIO3=01;DIO0=10--CADdone,DIO1=10--CadDetected
	SX1278_SPIWrite(module, REG_LR_DIOMAPPING2, 0x80);  //DIO4=10, DIO5=00,DIO4=PLLLOCK,DIO5=ModeReady

	SX1278_SPIWrite(module, LR_RegIrqFlagsMask, 0xfa);  //enable CADdone and CadDetected
	SX1278_SPIWrite(module, LR_RegIrqFlags, 0xff);		//clear interrupt

	SX1278_SPIWrite(module, LR_RegOpMode, 0x0F);        //enter CAD mode
